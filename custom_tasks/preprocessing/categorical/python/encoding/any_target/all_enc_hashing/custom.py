...
def fit(X, y, output_dir, **kwargs):
    ...
    As an output, this hook is expected to create an artifact containg a trained object [in this example - a pre-fit HashingEncoder], that is then used to transform new data.
    ...
    output_dir: str
        A path to the output folder; the artifact [in this example - a pickle file containing a pre-fit HashingEncoder] must be saved into this folder to be re-used in transform().
    ...
    encoder_hash = ce.HashingEncoder(cols=X.columns, hash_method='md5', max_process=2, n_components=16)
    encoder_hash.fit(X,y)
    ...
    if output_dir_path.exists() and output_dir_path.is_dir():
        with open("{}/hash.pkl".format(output_dir), "wb") as fp:
            pickle.dump(encoder_hash, fp)
...
def transform(data, transformer): 
    ...
    transformer: Any
        ...
        In this example, it's a pickle file containing a pre-fit HashingEncoder.
