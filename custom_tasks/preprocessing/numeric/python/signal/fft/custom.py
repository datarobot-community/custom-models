# Project: Custom Transform - Chebychev Filter
# Project Cerebro Hackathon Team
...
def fit(X, y, output_dir, **kwargs):
    ...
    As an output, this hook is expected to create an artifact containing a trained object [in this example - median of each numeric column], that is then used to transform new data.
    ...
    # Placeholder so thing works. Equation is just to test
    fourier = X.median(axis=0, numeric_only=True, skipna=True).to_dict()
...
def transform(data, transformer):
    ...
    for i in data.columns:
        df[i] = fft(np.array(df.loc[:, i])).astype(float)
